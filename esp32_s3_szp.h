...
typedef struct {
    ...
    float AngleX, AngleY, AngleZ;  // 当前倾角（Roll, Pitch, Yaw）
    float last_roll;                // 上一次Roll角度（用于互补滤波）
    float last_pitch;               // 上一次Pitch角度（用于互补滤波）
    float last_yaw;                 // 上一次Yaw角度（用于陀螺仪积分）
} t_sQMI8658;
...
#define XCLK_FREQ_HZ 10 * 1000 * 1000
