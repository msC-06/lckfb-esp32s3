...
#define COMPLEMENTARY_ALPHA   0.98f    // 互补滤波系数
#define LOWPASS_ALPHA         0.1f     // 低通滤波系数
#define DRIFT_COMPENSATION    0.0005f  // 漂移补偿系数（调整此值控制补偿强度）
#define GYRO_THRESHOLD        0.05f    // 陀螺仪静止检测阈值(°/s)
...
esp_err_t bsp_display_brightness_init(void)
{
    ...
    gpio_config_t io_conf = {
        ...
        .pull_up_en = GPIO_PULLUP_DISABLE,  // 关上拉
...
esp_err_t bsp_display_brightness_set(int brightness_percent)
{
    ...
    // 强制读回电平（debug）
    int level = gpio_get_level(BSP_LCD_BACKLIGHT);
    ESP_LOGI(TAG, "GPIO%d actual level: %d", BSP_LCD_BACKLIGHT, level);
...
esp_err_t bsp_touch_new(esp_lcd_touch_handle_t *ret_touch)
{
    ...
    // 暂时禁用触摸，防止崩溃
    // esp_err_t err = esp_lcd_touch_new_i2c_ft5x06(tp_io_handle, &tp_cfg, ret_touch);
    // if (err != ESP_OK) {
    //     *ret_touch = NULL;
    //     ESP_LOGW("touch", "触摸初始化失败，已跳过");
    // }
    return ESP_OK;
}
...
void bsp_lvgl_start(void)
{
    ...
    ESP_LOGI("LVGL", "开始 LVGL 初始化...");
    
    /* 初始化LVGL */
    lvgl_port_cfg_t lvgl_cfg = ESP_LVGL_PORT_INIT_CONFIG();
    ret = lvgl_port_init(&lvgl_cfg);
    if (ret != ESP_OK) {
        ESP_LOGE("LVGL", "lvgl_port_init 失败: %s", esp_err_to_name(ret));
        return;
    }
    ESP_LOGI("LVGL", "lvgl_port_init 成功");
    
    /* 初始化液晶屏 并添加LVGL接口 */
    ESP_LOGI("LVGL", "初始化液晶屏...");
    disp = bsp_display_lcd_init();
    if (disp == NULL) {
        ESP_LOGE("LVGL", "液晶屏初始化失败");
        return;
    }
    ESP_LOGI("LVGL", "液晶屏初始化成功");
    
    /* 初始化触摸屏 并添加LVGL接口 */
    ESP_LOGI("LVGL", "初始化触摸屏...");
    disp_indev = bsp_display_indev_init(disp);
    if (disp_indev == NULL) {
        ESP_LOGW("LVGL", "触摸屏初始化失败或未连接");
    } else {
        ESP_LOGI("LVGL", "触摸屏初始化成功");
    }
...
esp_err_t bsp_codec_init(void)
{
    ...
    record_dev_handle = NULL; // 暂时不使用麦克风，防止崩溃
